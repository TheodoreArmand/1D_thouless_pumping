...
pumpconfig::PumpConfig make_vs3_n2_base(const RcondCase& rc) {
    ...
    cfg.basis_from = "initial_state/Vs3Er_Vl3Er/initial_pathpad_N2_K32_Aseed0p05.csv";
    cfg.sigma_gauss = 1.0;
    return cfg;
}
...
std::string out_root_base(const RcondCase& rc) {
    return "out/vs3_n2_dt0p01_T160pi_K32_sweep_fine20_Aseed0p05_" + rc.tag;
}

int run_free(const RcondCase& rc) {
    pumpconfig::PumpConfig cfg = make_vs3_n2_base(rc);
    cfg.config_name = "legacy_prb_3_3_n2_pathpad_K32_Aseed0p05_dt0p01_fine20_" +
                      rc.tag + "_free";
    ...
    opt.config_appendix =
        "driver=ecg1d_vs3_n2_dt001_fine20_aseed_rcond_free\n"
        "hamiltonian_kinetic=1\n"
        "hamiltonian_delta=0\n"
        "hamiltonian_gaussian=0\n"
        "basis_variant=Aseed0p05_pathpads_only\n"
        "Aseed_alpha=0.05\n"
        "Aseed_terms=16-31\n"
        "Aseed_matrix=alpha[[1,-1],[-1,1]]\n"
        "manual_fine_dt_factor=0.05\n"
        "manual_fine_dt_windows_s=0.2-0.3,0.7-0.8\n"
        "rcond_case=" + rc.tag + "\n"
        "dt_planning_estimate_seconds_per_step=0.40\n"
        "dt_planning_estimate_steps=241279\n"
        "dt_planning_estimate_wall_hours=26.9\n";
...
int run_gauss(const RcondCase& rc, double sigma) {
    ...
    opt.config_appendix +=
        "driver=ecg1d_vs3_n2_dt001_fine20_aseed_rcond_gauss\n"
        "basis_variant=Aseed0p05_pathpads_only\n"
        "Aseed_alpha=0.05\n"
        "Aseed_terms=16-31\n"
        "Aseed_matrix=alpha[[1,-1],[-1,1]]\n"
        ...
        "manual_fine_dt_windows_s=0.2-0.3,0.7-0.8\n"
        "rcond_case=" + rc.tag + "\n"
        "dt_planning_estimate_seconds_per_step=0.85\n"
        "dt_planning_estimate_steps=241279\n"
        "dt_planning_estimate_wall_hours=57.0\n";
    return pump2::run_pump(cfg, opt);
}
...
int main(int argc, char** argv) {
    try {
        ...
        std::cerr << "FATAL vs3 N2 fine20 Aseed rcond run: " << e.what() << "\n";
        return 2;
    }
}
