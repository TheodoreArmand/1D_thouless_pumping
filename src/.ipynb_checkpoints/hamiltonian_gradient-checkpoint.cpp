...
// For kernels that don't need perm_matrix (rTr, G, H)
...
// ============================================================
// General cosine potential gradient
// ============================================================
    ...
    loop([&](const BasisParams& pi, const BasisParams& pj) -> Cd {
        ...
        for (int p = 0; p < SN; p++) {
            ...
            for (const auto& term : terms)
                kernel_vals[p] += term.C * compute_cosine_Mij(c, term.A, term.b0);
        }

        Cd dMG = addsn_dMG(a1, Real, pi, pj, a2, a3, a4, perms, kernel_vals, MG_vals);

        Cd dK(0, 0);
        if (a1 != 1 && (a2 == pi.name || a2 == pj.name)) {
            auto [idx1, idx2] = resolve_case_first(a2, pi.name, pj.name, Real);
            ...
            for (int p = 0; p < SN; p++) {
                ...
                for (const auto& term : terms) {
                    for (int pa = 0; pa < N; pa++) {
                        DerivTable dt;
                        if (a1 == 2) {
                            dt = dCosine_Mija_b(c, perms.sigmas[p], pa,
                                                term.A, term.b0, pi, pj);
                        } else if (a1 == 3) {
                            dt = dCosine_Mija_r(c, perms.sigmas[p], pa,
                                                term.A, term.b0, pi, pj);
                        } else {
                            dt = dCosine_Mija_a(c, perms.sigmas[p], pa,
                                                term.A, term.b0);
                        }
                        ...
                        dk_val += term.C * (dt[idx][idx1] + dt[idx_s][idx2]);
                    }
                }
                dK += static_cast<double>(perms.signs[p]) * MG_vals[p] * dk_val;
            }
