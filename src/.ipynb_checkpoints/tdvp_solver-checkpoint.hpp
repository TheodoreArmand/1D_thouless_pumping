...
struct HamiltonianTerms {
    ...
    static HamiltonianTerms kinetic_harmonic() {
        HamiltonianTerms t;
        t.delta = false;
        t.gaussian = false;
        return t;
...
struct SolverConfig {
    ...
    // In the real-time path (realtime_tdvp.cpp, compute_rhs_dz), hard
    // truncation drops directions of the solved matrix with
    // sv < rcond * sv_max. The imaginary-time solver below uses the same
    // relative rcond*sv_max convention.
