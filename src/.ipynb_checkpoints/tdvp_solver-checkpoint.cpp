...
MatrixXcd assemble_C(const std::vector<AlphaIndex>& alpha_z_list,
    ...
    // Each (a,b) entry is independent and writes a distinct slot of C_mat;
    // calculate_C reads only const inputs (no shared mutable state), so the
    // result is bitwise-identical regardless of thread count. collapse(2)
    // requires the two loops to be perfectly nested (alpha/beta inside body).
...
Cd grad_H_for_alpha(const AlphaIndex& alpha, bool Real,
    ...
    if (!terms.cosine_terms.empty())
        result += calculate_Hamiltonian_general_cosine_partial(
            alpha.a1, alpha.a2, alpha.a3, alpha.a4, Real, basis, terms.cosine_terms);
...
Cd compute_total_energy(const std::vector<BasisParams>& basis,
    ...
    if (!terms.cosine_terms.empty()) E_num += general_cosine_functional(basis, terms.cosine_terms);
    if (terms.one_body_constant != 0.0) E_num += terms.one_body_constant * N * S;
