...
// ============================================================
// 5. General cosine kernel derivatives
// ============================================================

static Cd d_cosine_from_le_lc(const PairCache& c, int a,
                              double A, double b0,
                              Cd d_le, Cd d_lc) {
    Cd le = c.K_inv(a, a);
    Cd lc = 2.0 * c.mu(a);
    Cd theta = b0 + 0.5 * A * lc;
    Cd exp_factor = std::exp(-0.25 * A * A * le);
    return exp_factor * (
        -0.25 * A * A * std::cos(theta) * d_le
        -0.5 * A * std::sin(theta) * d_lc);
}

DerivTable dCosine_Mija_b(const PairCache& c, const VectorXi& sigma, int a,
                          double A, double b0,
                          const BasisParams& pi, const BasisParams& pj) {
    int N = c.N;
    BasisParams pi_con = pi.conj_params();
    ...
    for (int i = 0; i < N; i++) {
        ...
        Cd case_2 = d_cosine_from_le_lc(c, a, A, b0, d_le_bra, d_lc_bra);

        Cd d_le_ket = -c.K_inv(a, i) * c.K_inv(i, a);
        Cd d_lc_ket = 2.0 * c.K_inv(a, i) * (pj.R(sigma_fu) - c.mu(i));
        Cd case_7 = d_cosine_from_le_lc(c, a, A, b0, d_le_ket, d_lc_ket);
...
DerivTable dCosine_Mija_r(const PairCache& c, const VectorXi& sigma, int a,
                          double A, double b0,
                          const BasisParams& pi, const BasisParams& pj) {
    int N = c.N;
    BasisParams pi_con = pi.conj_params();
    ...
    for (int i = 0; i < N; i++) {
        ...
        Cd case_2 = d_cosine_from_le_lc(c, a, A, b0, Cd(0), d_lc_bra);

        Cd d_lc_ket = 2.0 * c.K_inv(a, i) * pj.B(sigma_fu, sigma_fu);
        Cd case_7 = d_cosine_from_le_lc(c, a, A, b0, Cd(0), d_lc_ket);
...
DerivTable dCosine_Mija_a(const PairCache& c, const VectorXi& sigma, int a,
                          double A, double b0) {
    int N = c.N;
    DerivTable result;
    ...
    for (int m = 0; m < N; m++) {
        ...
        Cd case_6 = d_cosine_from_le_lc(c, a, A, b0, d_le_diag, d_lc_diag);
        Cd case_15 = case_6;
        ...
        for (int n = m + 1; n < N; n++) {
            ...
            Cd d_lc_off = -2.0 * (c.K_inv(a, m) * c.mu(n)
                                   + c.K_inv(a, n) * c.mu(m));
            Cd case_2 = d_cosine_from_le_lc(c, a, A, b0, d_le_off, d_lc_off);
            Cd case_11 = case_2;
